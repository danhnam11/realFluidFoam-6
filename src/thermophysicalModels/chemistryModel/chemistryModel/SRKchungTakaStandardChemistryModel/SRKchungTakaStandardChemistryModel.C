...
Foam::SRKchungTakaStandardChemistryModel<ReactionThermo, ThermoType>::SRKchungTakaStandardChemistryModel
(
...
:
    ...
    reactions_
    (
        dynamic_cast<const SRKchungTakaReactingMixture<ThermoType>&>(this->thermo())
    ),
    specieThermo_
    (
        dynamic_cast<const SRKchungTakaReactingMixture<ThermoType>&>
    ...
    dcdt_(nSpecie_)
{
    ...
    Info<< "SRKchungTakaStandardChemistryModel: Number of species = " << nSpecie_
...
template<class ReactionThermo, class ThermoType>
Foam::SRKchungTakaStandardChemistryModel<ReactionThermo, ThermoType>::
...
template<class ReactionThermo, class ThermoType>
void Foam::SRKchungTakaStandardChemistryModel<ReactionThermo, ThermoType>::omega
(
    const scalarField& c,
    const scalar T,
    const scalar p,
...
template<class ReactionThermo, class ThermoType>
Foam::scalar Foam::SRKchungTakaStandardChemistryModel<ReactionThermo, ThermoType>::omegaI
(
    const label index,
    const scalarField& c,
    const scalar T,
...
template<class ReactionThermo, class ThermoType>
void Foam::SRKchungTakaStandardChemistryModel<ReactionThermo, ThermoType>::derivatives
(
    const scalar time,
    const scalarField& c,
    scalarField& dcdt
) const
{
    ...
    //Info << "   SRKchung...Chem., derivative: before omega, T = " << T << endl; //    
    omega(c_, T, p, dcdt);
    //Info << "   SRKchung...Chem., derivative: after omega, T = " << T << endl; //    
    ...
    for (label i=0; i<nSpecie_; i++)
    {
        if (T < 0)
        {        
        Info << " In SRK...chemistryModel, derivatives function" << endl;
        Info << " In derivatives func, before call cp(p, T), T =  " << T << endl;
        }
...
template<class ReactionThermo, class ThermoType>
void Foam::SRKchungTakaStandardChemistryModel<ReactionThermo, ThermoType>::jacobian
(
    const scalar t,
    const scalarField& c,
    scalarField& dcdt,
...
Foam::SRKchungTakaStandardChemistryModel<ReactionThermo, ThermoType>::tc() const
{
...
template<class ReactionThermo, class ThermoType>
Foam::tmp<Foam::volScalarField>
Foam::SRKchungTakaStandardChemistryModel<ReactionThermo, ThermoType>::Qdot() const
{
...
Foam::SRKchungTakaStandardChemistryModel<ReactionThermo, ThermoType>::calculateRR
(
    const label ri,
    const label si
) const
...
template<class ReactionThermo, class ThermoType>
void Foam::SRKchungTakaStandardChemistryModel<ReactionThermo, ThermoType>::calculate()
{
    if (!this->chemistry_)
    {
        return;
...
Foam::scalar Foam::SRKchungTakaStandardChemistryModel<ReactionThermo, ThermoType>::solve
(
...
)
{
    ...
    forAll(rho, celli)
    {
        ...
        if(Ti < 0) //Nam
        {
          Info << " in solve func. SRK...chemistryModel, T[celli] = " << Ti << endl;  
        }
