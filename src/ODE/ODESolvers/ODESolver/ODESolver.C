...
{
    scalar x0 = x;
    Info << "   ---> in solve #1, before" << endl; //
    solve(x, y, step.dxTry);
    Info << "   ---> in solve #1, after" << endl; //
    step.dxDid = x - x0;
}
...
) const
{
    ...
    for (label nStep=0; nStep<maxSteps_; nStep++)
    {
        ...
        Info << " ODESolver class, before solve(x, y, step) " << endl; //
        solve(x, y, step);
        Info << " ODESolver class, after solve(x, y, step) " << endl; //
