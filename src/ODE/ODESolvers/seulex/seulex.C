...
) const
{
    ...
    Info << "   - in seul: #1, T = " << y0[9]  << endl; //
    odes_.derivatives(xnew, y0, dy_);
    Info << "   - in seul: #2, T = " << y0[9]  << endl; //

    LUBacksubstitute(a_, pivotIndices_, dy_);
    Info << "   - in seul: #2.2, T = " << y0[9]  << endl; //

    yTemp_ = y0;

    for (label nn=1; nn<nSteps; nn++)
    {
            Info << "   - in seul: #2.3, T = " << yTemp_[9] << endl;   //
        yTemp_ += dy_;
            Info << "   - in seul: #2.4, T = " << yTemp_[9] << endl;   //
        xnew += dx;
        ...
        if (nn == 1 && k<=1)
        {
            ...
            Info << "   - in seul: #3, T = " << yTemp_[9] << endl;   //
            odes_.derivatives(x0 + dx, yTemp_, dydx_);
            Info << "   - in seul: #4, T = " << yTemp_[9] << endl;   //
...
{
    //Info << "   *** in seulex class: inside solve(x,y,step): #1" << endl; //
    ...
    //Info << "   *** in seulex class: inside solve(x,y,step): #2" << endl; //
    while (firstk || step.reject)
    {
        ...
        scalar errOld = 0;
    //Info << "   *** in seulex class: inside solve(x,y,step): #3" << endl; //

        for (k=0; k<=kTarg_+1; k++)
        {
    Info << "   *** in seulex class: inside solve(x,y,step): #4.1" << endl; //
            bool success = seul(x, y0_, dx, k, ySequence_, scale_);
    Info << "   *** in seulex class: inside solve(x,y,step): #4.2" << endl; //
        ...
        if (step.reject)
        {
    //Info << "   *** in seulex class: inside solve(x,y,step): #5" << endl; //
    ...
    //Info << "   *** in seulex class: inside solve(x,y,step): #6" << endl; //
    jacUpdated = false;
